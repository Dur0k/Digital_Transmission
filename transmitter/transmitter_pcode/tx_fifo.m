...
if isempty(buffer)
    buffer=ones(1,par_fifolen)*NaN; %buffer der richtigen länge, gefüllt mit NaNs
end
...
if (num_nan)>=length(b) %prüfen ob genügend platz ist im buffer, um b einzufügen
    buffer_inv(idb:idb+length(b)-1)=b; %b eingeschoben auf platz 1-8 mit erstem wert ganz hinten
    buffer=flip(buffer_inv); %buffer mit eingeschoebenem b wieder zurück invertieren
end
...
if n>=par_ccblklen % anzahl integers im buffer summieren (ausgabe=ans=anzahl stellen, die eine ziffer enthalten statt Nan)
    b_buf_inv=buffer_inv(1:par_ccblklen); %erste werte des invertierten buffers auslesen (entsprechen den letzten des buffers)
    [b_buf]=flip(b_buf_inv)'; %ausgelesene werte in die richtige reihenfolge bringen
